...
var current_shadow: Shadow = null   # 当前可交互物（可选）
...
func enter_shadow(shadow: Shadow) -> void:
	current_shadow = shadow
	# 这里也可以通知 HUD：show hold bar
	print("进入shadow交互范围：", shadow.name)

func exit_shadow(shadow: Shadow) -> void:
	if current_shadow == shadow:
		current_shadow = null
		# 也可以通知 HUD：hide
		print("离开shadow交互范围：", shadow.name)
